...
@Controller
public class PayForOrderController implements MessageSourceAware  {
	@Autowired
	    ...
	    public String viewdoneorders(Locale locale, Model model, final Principal principal) {
		 log.info("Here you can view done orders: "+ locale.toString());
	      ...
	      //  model.addAttribute("doneOrder",
	        ...
	        for (Order o:os.fetchAllUndoneOrderById(userSvc.getUserByLogin(principal.getName()).getClient().getClientId())){
...
//	       }
	 ...
	 public String pay(Locale locale, Model model, final Principal principal, @RequestParam("btn") Integer no) throws SQLException {
		log.info(no);
		 ...
		 machineSvc.repair(os.fetchOrderById(no).getMachine_id(), machineSvc.fetchMachineById(os.fetchOrderById(no).getMachine_id()).getTimes_repaired()+1);
		 log.info("PAYED!!!: "+  locale.toString());
			return "redirect:/payfororder";
	 }
}
