...
<title>MachineRepair View Done Orders</title>
<style type="text/css">
	</style>
<style>
.error {
...
<tr><th>NUMBER</th> <th>DATE</th><th>NAME</th> <th>PRICE($)</th> <th>DURATION(Hrs)</th> <th>STATUS</th> <th>ACTION</th>
</tr>
<c:forEach var="ms" items="${orderMachineRepair}"  varStatus="loopStatus">  
 <tr class="${loopStatus.index % 2 == 0 ? 'even' : 'odd'}">
    	<td><c:out value="${loopStatus.index + 1}"/></td>
    	  <td>${ms.start}</td>  <td> ${ms.name}</td> 
    	  <td> ${ms.price}</td> <td> ${ms.duration}</td> <td> ${ms.status}</td> <td>
    	  <button name="btn"
    	  value=${ms.order_id}>Pay</button></td></tr>  	
    </c:forEach>    
    </table>
    </form>
</body>
